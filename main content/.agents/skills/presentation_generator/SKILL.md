...
> 🚨 **完全自律実行**: Phase 1〜3を一切の確認・問い返しなく最後まで自律的に実行する。フェーズ間でユーザーに「次に進みますか？」と聞くのは**禁止**。検証エラーは自分で修正し、全フェーズ完了後に最終報告のみ行う。
...
台本に `<!-- SCENE: ビジュアルパターン | シーン見出し -->` マーカーが記載されている場合、HTMLの `<section>` シーンは台本のSCENEマーカーと **完全に1:1で対応** しなければならない。

- 台本にN個のSCENEマーカーがあれば、HTMLにもちょうどN個の `<section>` を作成すること
- HTMLのシーン順序は台本のSCENEマーカーの出現順序と完全に一致させること
- 各HTMLシーンの `<!-- Scene N: ... -->` コメントに、対応する台本SCENEマーカーの見出しを含めること

**以下は絶対に禁止:**
- 台本にないシーンの独自追加（例: 「テキスト強調 問いかけ」「タイトルカード まとめ」「エンディング」等）
- 台本の2つのSCENEマーカーを1つのHTMLシーンに統合
- 台本のSCENEマーカーの順序を変更

> 💡 HTMLシーン数が台本SCENE数と一致しないと、scene_map.jsonによるセリフ→シーンのマッピングがズレ、映像と音声の同期が壊れる。
...
各シーンは、台本の内容に応じて以下のビジュアルパターンから選択・組み合わせて構築する。パターンは台本の `<!-- SCENE: ビジュアルパターン | シーン見出し -->` マーカーで指定されることが多いが、指定がない場合は内容に最適なパターンを自分で選択する。
...
## Step 3：`scene_map.json` を自動生成する

台本のセリフをシーンに対応付けた `scene_map.json` を生成する。**これは Phase 2〜3 の全工程の入力データ**であり、正確に作成する必要がある。
...
```
❌ 間違い1: HTMLにタイトルシーンがある場合に、scene_mapでhold_sec付きの
   無音シーンを「追加」し、台本セリフをScene 1以降に入れる
   → セリフと映像が全体的に1シーンずつズレる

❌ 間違い2: 台本のメタデータ行（例: 「シーン遷移マーカー: あり」の説明文）に
   <!-- SCENE: ... --> と書いてしまい、正規表現がダミーシーンを検出する
   → 先頭にセリフ空のScene 0が生まれ、以降すべてズレる
```
...
✅ HTMLにセリフなしのエンディングシーンがある場合は、
   scene_mapの最後に hold_sec 付きのエントリを追加する
```
...
以下のNode.jsスクリプトで、台本から scene_map.json を自動生成する。

**使い方**: 台本パスとプロジェクトディレクトリを指定して実行する。
...
lines.forEach(l => {
  // <!-- SCENE: ... --> にマッチ。ただしバッククォート内やインラインコード内は除外
  if (l.includes('`') && l.includes('SCENE:')) return;  // メタデータ行のスキップ
...
// titleからビジュアルパターン名を除去し、パイプ右側の見出しを抽出
...
// HTMLシーン数 > 台本シーン数 の場合、差分をエンディング等のhold_secシーンとして追加
...
console.log('   台本 SCENE markers: ' + sceneMarkers.length);
...
> ⚠️ `{台本DIR}` と `{MC_DIR}` は実際のパスに置き換えること。
...
| `title` | シーンの**コンテンツタイトル**（チャプター表示用）。台本マーカーの「」括弧内のテキストのみ。ビジュアルパターン名（タイトルカード等）は含めない |
| `hold_sec` | **セリフがない**シーンの表示固定時間（秒）。セリフありのシーンには不要 |
| `lines` | そのシーンで読み上げるセリフの配列（順序 = 読み上げ順） |
| `lines[].speaker` | `speakers` で定義した話者名 |
| `lines[].text` | 読み上げテキスト |
| `speed_scale` | 読み上げ速度（1.14推奨） |
| `inter_line_silence` | セリフ間の無音秒数（0.3推奨） |
| `scene_end_padding` | シーン末尾パディング秒数（0.5推奨） |

### 生成後の検証手順（必須）

scene_map.json を生成したら、以下を **必ず** 確認すること：
...
## Step 3.5：VOICEVOX ユーザー辞書に読みを登録する（必須）

音声生成の前に、台本中の英語・専門用語の読みを VOICEVOX のユーザー辞書に登録する。
これにより、英語略語・人名・医学用語などの読み間違えを防止できる。
...
**このコマンドが行うこと:**
1. `scene_map.json` の全セリフテキストを結合
2. 組み込み辞書（`register_dict.js` 内の `BUILTIN_DICT`）から、テキスト中に出現する単語を検出
3. プロジェクト固有辞書（`{テーマ名}/.voicevox_dict.json`）があればそれも読み込む
4. 既に VOICEVOX に登録済みの単語はスキップし、未登録のもののみ API で登録

**プロジェクト固有辞書（`.voicevox_dict.json`）のフォーマット:**

```json
[
    { "surface": "遅発性筋肉痛", "pronunciation": "チハツセイキンニクツー", "accent_type": 1 },
    { "surface": "糖新生", "pronunciation": "トウシンセイ", "accent_type": 3 }
]
```

| フィールド | 説明 |
|-----------|------|
| `surface` | 台本上の表記（そのまま一致検索される） |
| `pronunciation` | カタカナ読み（VOICEVOX が認識する形式） |
| `accent_type` | アクセント核の位置（0=平板、1〜=核の位置） |
| `priority` | 優先度（省略時 5、1=低〜10=高） |

> ⚠️ **組み込み辞書 `BUILTIN_DICT` の拡充**: 新プロジェクトで頻出する一般的な英語略語・学術用語があれば、`register_dict.js` 内の `BUILTIN_DICT` に追加して全プロジェクトで再利用できるようにすること。
...
| シーンカウントが合わない | `class="scene dark-scene"` が正規表現でマッチしない | IDベースでカウントする |
| Scene 0が一瞬で飛ぶ | scene_mapにダミーScene 0が生成されている | 台本メタデータ行のSCENEマーカー記載を確認。自動生成スクリプトを使用 |
| 先頭シーンの映像と音声がズレる | scene_map ID 0にhold_sec付き無音シーンを追加してしまった | 台本最初のSCENEのセリフはScene 0のlinesに入れる。余分なシーンを追加しない |
