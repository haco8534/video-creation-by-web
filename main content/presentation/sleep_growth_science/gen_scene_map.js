...
const SCRIPT_PATH = 'd:/myfolder/動画生成/台本作成/「寝る子は育つ」は科学的に正しかった/script.md';
const PROJECT_DIR = 'd:/myfolder/動画生成/main content/presentation/sleep_growth_science';
...
const allDialogue = lines
    ...
    .map(l => { const [name, ...rest] = l.split('：'); return { speaker: name.trim(), text: rest.join('：').trim() }; });
...
if (scenes.length !== htmlSceneCount) {
    console.log('WARNING: scene_map entries (' + scenes.length + ') != HTML scenes (' + htmlSceneCount + ')');
}
