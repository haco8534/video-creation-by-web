...
const SCRIPT_PATH = path.resolve('d:/myfolder/動画生成/ふぁくとラボ/台本作成/嘘発見器は嘘を検出できない/script.md');
const PROJECT_DIR = path.resolve('d:/myfolder/動画生成/ふぁくとラボ/main content/presentation/lie_detector_myth');
const HTML_PATH = path.join(PROJECT_DIR, 'index.html');
const OUT_PATH = path.join(PROJECT_DIR, 'scene_map.json');
const script = fs.readFileSync(SCRIPT_PATH, 'utf8');
const lines = script.split('\n');
const allDialogue = lines.filter(l => /^(ずんだもん|めたん)：/.test(l)).map(l => { const [name, ...rest] = l.split('：'); return { speaker: name, text: rest.join('：').trim() }; });
...
else console.log('OK: scene count matches');
