...
このワークフローは **テーマ入力後、Phase A → Phase B → Phase C → Phase D を一切の確認・問い返しなく最後まで自律的に実行する**。

**禁止事項:**
- ❌ フェーズ完了後に「次に進みますか？」「続けてもよいですか？」と聞くこと
- ❌ 検証結果を報告して「確認してください」と待つこと
- ❌ 中間成果物を見せて承認を求めること
- ❌ Phase間で一時停止してユーザーの返答を待つこと

**必須事項:**
- ✅ 検証でエラーがあれば **自分で修正して再検証** し、自動で次に進む
- ✅ 全フェーズを一気通貫で実行し、**最終報告のみ** をユーザーに返す
- ✅ VOICEVOXに接続できない等の **ユーザーの物理的アクションが必要な場合のみ** 問い合わせる

> パイプライン全体で **ユーザーの承認が必要な箇所はゼロ**。テーマ入力から完成品フォルダ整理まで、すべて自動実行される。
